...
{
    enum_t ver_enum = add_enum(BADADDR, "version_reg", hex_flag());
...
int idaapi accept_file(qstring *fileformatname, qstring *processor, linput_t *li, const char *filename)
{
    ...
    fileformatname->sprnt("Sega Genesis/MegaDrive ROM file v.2");
...
{
    if (ph.id != PLFM_68K) {
        set_processor_type(M68K, SETPROC_LOADER); // Motorola 68000
