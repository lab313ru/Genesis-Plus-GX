...
static int show_transparence = 0;
...
static void PlaneExplorer_DrawTile(unsigned short name_word, unsigned int x, unsigned int y, int transcolor)
{
    ...
    int tile_height = ((reg[12] & 0x6) == 6) ? 16 : 8;
...
static void PlaneExplorer_GetSpriteInfo(unsigned short *plane, unsigned char currentSpriteNo, sprite_info *sprite)
{
    ...
    sprite->priority = ((plane[offset + 2] >> 15) & 1) ? 1 : 0;
    sprite->paletteLine = (plane[offset + 2] >> 13) & 3;
    sprite->vflip = ((plane[offset + 2] >> 12) & 1) ? 1 : 0;
    sprite->hflip = ((plane[offset + 2] >> 11) & 1) ? 1 : 0;
...
static void CalculateEffectiveCellScrollSize(unsigned char *effectiveScrollWidth, unsigned char *effectiveScrollHeight,
    unsigned char *tileWidthInPixels, unsigned char *tileHeightInPixels, int *h40ModeActive, int *interlaceMode2Active)
{
    unsigned int hszState = reg[16] & 3;
    unsigned int vszState = (reg[16] >> 4) & 3;
    ...
    *tileWidthInPixels = 8;
    *tileHeightInPixels = ((reg[12] & 0x6) == 6) ? 16 : 8;
    *h40ModeActive = (reg[12] & 1) ? 1 : 0;
    *interlaceMode2Active = ((reg[12] & 6) == 6) ? 1 : 0;
...
static void PlaneExplorer_DrawSprite(sprite_info sprite, unsigned char tileWidthInPixels, unsigned char tileHeightInPixels, int h40ModeActive, int interlaceMode2Active)
{
    ...
    for (unsigned char j = 0; j < spriteHeightInCells * tileHeightInPixels; j++)
    {
        for (unsigned char i = 0; i < spriteWidthInCells * tileWidthInPixels; i++)
        {
            ...
            int patternDataUpperHalf = (patternColumnNo % pixelsPerPatternByte) == 0;
...
{
    int h40ModeActive, interlaceMode2Active;
    ...
    case 1: // B
        ...
        break;
    case 2: // W
        base = (reg[3] & (h40ModeActive ? 0x3C : 0x3E)) << 10;
        break;
    case 3: // S
        base = (reg[5] & (h40ModeActive ? 0x7E : 0x7F)) << 9;
        break;
    }
